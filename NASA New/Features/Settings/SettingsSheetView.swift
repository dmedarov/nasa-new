...
private struct SettingsNetworkDiagnosticsSection: View {
    ...
    private var videoAutoplayPolicyLabel: String {
        ...
        guard !wifiOnlyVideoAutoplay else {
            if !networkReachable {
                return L10n.text("video.waiting_for_network", default: "Waiting for network connection")
            }
        ...
        return L10n.text("video.autoplay_any_network", default: "Autoplay allowed on any network")
    }
    ...
    var body: some View {
        Section(L10n.text("Network Diagnostics", default: "Network Diagnostics")) {
            ...
            if wifiOnlyVideoAutoplay {
                Text(
                    L10n.text(
                        "Videos will wait for manual playback unless the device is on Wi-Fi.",
                        default: "Videos will wait for manual playback unless the device is on Wi-Fi."
                    )
                )
                .font(AppTheme.Typography.footnote)
                .foregroundColor(.secondary)
...
private struct SettingsContentPreferencesSection: View {
...
private struct SettingsRecentRequestsSection: View {
...
private struct SettingsNotificationsSection: View {
    ...
    let nextScheduledNotificationText: String
    ...
    var body: some View {
        Section(L10n.text("Daily Notifications", default: "Daily Notifications")) {
            ...
            LabeledContent(L10n.text("Next Scheduled Alert", default: "Next Scheduled Alert")) {
                Text(nextScheduledNotificationText)
                    .multilineTextAlignment(.trailing)
            }
...
private struct SettingsStoragePreferencesSection: View {
...
private struct SettingsStorageDiagnosticsSection: View {
    ...
    var body: some View {
        AdaptiveNavigationContainer {
            Form {
                ...
                SettingsContentPreferencesSection(
                    preferImages: $preferImages,
                    allowVideoPlayback: $allowVideoPlayback
                )

                SettingsAPIDiagnosticsSection(
                    lastStatusCode: lastStatusCode,
                    lastRequestDateText: formattedRequestDate(lastRequestDate),
                    isAPIKeyConfigured: isAPIKeyConfigured,
                    lastTransportError: lastTransportError,
                    isUsingCachedData: isUsingCachedData,
                    apiKeyWarning: apiKeyWarning,
                    rateLimitRetryDateText: rateLimitRetryDate.map(formattedRequestDate)
                )

                if !diagnosticsHistory.isEmpty {
                    SettingsRecentRequestsSection(diagnosticsHistory: diagnosticsHistory)
                }
                ...
                SettingsNotificationsSection(
                    ...
                    nextScheduledNotificationText: formattedRequestDate(nextScheduledNotificationDate)
                )

                SettingsStoragePreferencesSection(
                ...
                SettingsStorageDiagnosticsSection(
                    ...
                    appliedCacheItemLimit: appliedCacheItemLimit,
                    dataSaverMode: dataSaverMode,
                    preferHDImages: preferHDImages
                )
                ...
                SettingsNetworkDiagnosticsSection(
                    networkConnectionLabel: networkConnectionLabel,
                    ...
                    allowVideoPlayback: allowVideoPlayback,
                    wifiOnlyVideoAutoplay: wifiOnlyVideoAutoplay,
                    videoAutoplayEligible: videoAutoplayEligible
                )
